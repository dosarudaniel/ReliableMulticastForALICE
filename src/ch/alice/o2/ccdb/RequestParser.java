...
/**
 * Parse the request parameters and make the constraints available to the
 * application
 *
 * @author costing
 * @since 2017-09-22
...
public class RequestParser {
    ...
    /**
     * End validity of the request (default is `now` - an open end interval, so
     * +1ms)
     */
    ...
    /**
     * Snapshot timestamp (the cutoff time after which any newer created object is
     * ignored)
     */
    ...
    /**
     * If just the latest object version is requested, default is <code>true</code>
     * for retrieving or getting the latest matching set. It is automatically set to
     * <code>false</code> when the request is for /browse/..., in order to list all
     * matching objects.
     */
    ...
    /**
     * @param request request to wrap around
     */
    ...
    /**
     * @param request                 request to wrap around
     * @param optionalTimeConstraints whether or not the time constraints are
     *                                required. If <code>true</code> then at least
     *                                one time parameter has to be indicated (start
     *                                time / validity moment). If <code>false</code>
     *                                then any object in that interval will match.
     */
	...
	// search for path tokens, stop at the first numeric value which would be the
	// start time
	...
	if (pathElements.size() < (optionalTimeConstraints ? 0 : 1) || pathElements.size() > 10
		|| (!optionalTimeConstraints && !startTimeSet)) {
	    ok = false;
	    return;
	}
