...
/**
 * SQL-backed implementation of CCDB. Files reside on this server and/or a
 * separate storage and clients are served directly or redirected to one of the
 * other replicas for the actual file access.
 *
 * @author costing
 * @since 2017-10-13
 */
...
public class SQLBacked extends HttpServlet {
     ...
     */
    public static final String basePath = Options.getOption("file.repository.location",
	    System.getProperty("user.home") + System.getProperty("file.separator") + "QC");
	...
	try {
		...
		response.sendError(HttpServletResponse.SC_BAD_REQUEST,
			"POST request doesn't contain the data to upload");
		return;
	    }
		...
		response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
			"Cannot create target file to perform the upload");
		return;
	    }
		...
		response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
			"Cannot upload the blob to the local file " + targetFile.getAbsolutePath());
		targetFile.delete();
		return;
	    }
		...
		response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
			"Cannot insert the object in the database");
		return;
	    }
	...
	try {
		...
		response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
			"Could not delete the underlying record");
		return;
	    }
	...
	try (DBFunctions db = SQLObject.getDB()) {
		...
		// db.query("CREATE INDEX IF NOT EXISTS ccdb_validFrom_idx ON
		// ccdb(validFrom);");
		// db.query("CREATE INDEX IF NOT EXISTS ccdb_validUntil_idx ON
		// ccdb(validUntil);");
		// db.query("CREATE INDEX IF NOT EXISTS ccdb_createTime_idx ON
		// ccdb(createTime);");
		...
		db.query(
			"CREATE TABLE IF NOT EXISTS ccdb_paths (pathId SERIAL PRIMARY KEY, path text UNIQUE NOT NULL);");
		db.query(
			"CREATE TABLE IF NOT EXISTS ccdb_metadata (metadataId SERIAL PRIMARY KEY, metadataKey text UNIQUE NOT NULL);");
		db.query(
			"CREATE TABLE IF NOT EXISTS ccdb_contenttype (contentTypeId SERIAL PRIMARY KEY, contentType text UNIQUE NOT NULL);");
		db.query("CREATE TABLE IF NOT EXISTS config(key TEXT PRIMARY KEY, value TEXT);");

		if (!db.query("SELECT * FROM ccdb LIMIT 0")) {
		    System.err.println(
			    "Database communication cannot be established, please fix config.properties and/or the PostgreSQL server and try again");
		    System.exit(1);
		}
		...
		db.query(
			"CREATE TABLE IF NOT EXISTS ccdb_stats (pathid int primary key, object_count bigint default 0, object_size bigint default 0);");
		...
		db.query(
			"CREATE TRIGGER ccdb_increment_trigger AFTER INSERT ON ccdb FOR EACH ROW EXECUTE PROCEDURE ccdb_increment();",
			true);
		db.query(
			"CREATE TRIGGER ccdb_decrement_trigger AFTER DELETE ON ccdb FOR EACH ROW EXECUTE PROCEDURE ccdb_decrement();",
			true);
	    } else
