...
/**
 * SQL-backed implementation of CCDB. This servlet only implements GET (and
 * HEAD) for a particular UUID that is known to reside on this server. It should
 * normally not be accessed directly but clients might end up here if the file
 * was not migrated to other physical locations.
 *
 * @author costing
 * @since 2017-10-13
 */
...
public class SQLDownload extends HttpServlet {
	...
	try {
	    ...
	    response.sendError(HttpServletResponse.SC_BAD_REQUEST,
		    "The only path information supported is the requested object ID, i.e. '/download/UUID'");
	    return;
	}
	...
	if (matchingObject == null) {
	    response.sendError(HttpServletResponse.SC_NOT_FOUND,
		    "Object id " + id + " could not be found in the database");
	    return;
	}
    ...
    static void download(final boolean head, final SQLObject obj, final HttpServletRequest request,
		...
		if (start < 0) {
		    ...
		    response.sendError(HttpServletResponse.SC_REQUESTED_RANGE_NOT_SATISFIABLE,
			    "You requested more last bytes (" + s.substring(idx + 1)
				    + ") from the end of the file than the file actually has (" + obj.size + ")");
		    start = 0;
		}
	    } else {
	...
	resp.sendError(HttpServletResponse.SC_NOT_IMPLEMENTED,
		"The DELETE method should use the main entry point instead of /download/, which is reserved for direct read access to the objects");
    }
    ...
    protected void doPost(final HttpServletRequest req, final HttpServletResponse resp)
	    throws ServletException, IOException {
	resp.sendError(HttpServletResponse.SC_NOT_IMPLEMENTED,
		"You shouldn't try to create objects via the /download/ servlet, go to / instead");
    }
    ...
    protected void doPut(final HttpServletRequest req, final HttpServletResponse resp)
	    throws ServletException, IOException {
	resp.sendError(HttpServletResponse.SC_NOT_IMPLEMENTED,
		"Object manipulation is not available via the /download/ servlet, go to / instead");
    }
}
