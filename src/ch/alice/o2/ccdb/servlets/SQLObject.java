	...
	if (!existing || tainted) {
		...
		if (existing) {
		    final boolean ok = db.query(
			    ...
			    getContentTypeID(contentType, true), metadata, Long.valueOf(lastModified), id);
		    ...
		    if (db.query(
			    ...
			    false, id, pathId, Double.valueOf(validFrom / 1000.), Double.valueOf(validUntil / 1000.),
			    Long.valueOf(createTime), replicaArray, Long.valueOf(size), md5,
			    Long.valueOf(initialValidity), fileName, getContentTypeID(contentType, true), uploadedFrom,
			    metadata, Long.valueOf(lastModified))) {
			existing = true;
			tainted = false;
			return true;
		    }
