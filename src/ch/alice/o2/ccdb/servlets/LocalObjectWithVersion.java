...
/**
 * Handle all interactions with a local file, including the metadata access
 * (backed by a .properties file with the same base name as the referenced file)
 *
 * @author costing
 * @since 2017-09-21
 */
public class LocalObjectWithVersion implements Comparable<LocalObjectWithVersion> {
    ...
    /**
     * Metadata keys and values, stored in an additional
     * <i>{@link #referenceFile}.properties</i> file next to the blob object.
     */
    ...
    /**
     * Whether or not the end time was fully inferred already, from reading the
     * .properties file if necessary.
     */
    private boolean completeEndTime = true;

    private final static ExpirationCache<String, Long> validityInterval = Local.hasMillisecondSupport() ? null
	    : new ExpirationCache<>(65536);
    ...
    /**
     * @return the start of validity interval, in epoch millis. Interval contains
     *         this value.
     */
    ...
    /**
     * @return the end of validity interval, in epoch millis. Interval excludes this
     *         value.
     */
	...
	// we can't trust the filesystem to keep accurate modification times, we'll have
	// to load it from the metadata
     ...
     * @param key
     * @param defaultValue
     * @return the value of this key, either from the metadata or falling back to
    ...
    /**
     * @param referenceTime
     * @return <code>true</code> if the reference time falls between start time
    ...
    /**
     * @return the end of validity interval as set by the initial upload. In epoch
     *         millis.
     */
    ...
    /**
     * @return the last time when the object data or metadata was modified. In epoch
     *         millis.
     */
	...
	try {
		...
		// if everything else fails use as last resort the start time of the interval,
		// normally these two are the same
		return startTime;
	    }
	}
    ...
    /**
     * @param endTime the new end validity interval
     */
	...
	sb.append("Validity: ").append(getStartTime()).append(" - ").append(getEndTime()).append(" (")
		.append(new Date(getStartTime())).append(" - ").append(new Date(getEndTime())).append(")\n");
	sb.append("Initial validity limit: ").append(getInitialValidity()).append(" (")
		.append(new Date(getInitialValidity())).append(")\n");
	sb.append("Created: ").append(createTime).append(" (").append(new Date(createTime)).append(")\n");
	sb.append("Last modified: ").append(getLastModified()).append(" (").append(new Date(getLastModified()))
		.append(")\n");
	sb.append("Original file: ").append(getOriginalName()).append(", size: ").append(getSize()).append(", md5: ")
		.append(getProperty("Content-MD5"));
	...
	if (objectProperties != null && objectProperties.size() > 0) {
		...
		sb.append("  ").append(key.toString()).append(" = ")
			.append(objectProperties.getProperty(key.toString())).append('\n');
	}
