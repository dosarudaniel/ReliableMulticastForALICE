...
public class SQLBrowse extends HttpServlet {
	...
	/*
	 * if (!parser.ok) { printUsage(request, response); return; }
	 */
	...
	try (PrintWriter pw = response.getWriter()) {
		...
		try (DBFunctions db = SQLObject.getDB()) {
			...
			db.query("select distinct split_part(path,'/'," + (cnt + 2)
				+ ") from ccdb_paths where path like '" + Format.escSQL(parser.path)
				+ "/%' order by 1;");
			...
			if (sizeReport) {
				...
				db2.query(
					"SELECT object_count, object_size FROM ccdb_stats WHERE pathid=(SELECT pathid FROM ccdb_paths WHERE path='"
						+ prefix + db.gets(1) + "');");
				...
				db2.query(
					"SELECT sum(object_count), sum(object_size) FROM ccdb_stats WHERE pathid IN (SELECT pathid FROM ccdb_paths WHERE path LIKE '"
						+ prefix + db.gets(1) + "/%');");
				...
				formatter.subfoldersListing(pw, prefix + db.gets(1), prefix + db.gets(1) + suffix,
					ownCount, ownSize, subfoldersCount, subfoldersSize);
			    }
			} else
			    formatter.subfoldersListing(pw, prefix + db.gets(1), prefix + db.gets(1) + suffix);
		    }
