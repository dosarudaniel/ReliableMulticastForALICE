...
public class EmbeddedTomcat extends Tomcat {
    ...
    public final StandardContext ctx;

    /**
     * @param defaultAddress default listening address for the Tomcat server. Either
     *                       "localhost" (default for testing servers) or "*" for
	...
	// This is to disable Tomcat from creating work directories, nothing needs to be
	// compiled on the fly
	...
	getConnector().setProperty("maxKeepAliveRequests",
		String.valueOf(Options.getIntOption("maxKeepAliveRequests", 1000)));

	getConnector().setProperty("connectionTimeout",
		String.valueOf(Options.getIntOption("connectionTimeout", 10000))); // clients should be quick

	getConnector().setProperty("disableUploadTimeout", "false");
	getConnector().setProperty("connectionUploadTimeout",
		String.valueOf(Options.getIntOption("connectionTimeout", 300000))); // 5 minutes max to upload an object
	...
	if (getService().findConnectors()[0].getState() == LifecycleState.FAILED) {
	    ...
	    if (debugLevel < 2)
		System.err.println(
			"Set -Dtomcat.debug=2 (or export TOMCAT_DEBUG=2) to see the logging messages from the server.");
