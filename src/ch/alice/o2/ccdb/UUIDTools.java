...
/**
 * Helper class to generate version 1 (time based) UUIDs. This class doesn't
 * implement fully the object since the MAC address is hardcoded to a fixed
 * value. Since for this project the objects are generated on the server side
 * and thus the MAC address would be that of the server, this field is anyway
 * not relevant.
 *
 * @author costing
 * @since 2017-10-02
...
public class UUIDTools {
    ...
    /**
     * @param referenceTime timestamp to be encoded in the UUID
     * @param address       client's address to encode in the UUID. Up to 6 bytes
     *                      from it will be placed in the MAC address field. If not
     *                      provided (or less than 6 bytes long, ie. IPv4) then a
     *                      fixed value will be used for the missing part.
